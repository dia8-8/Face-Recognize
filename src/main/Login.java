...
public class Login extends javax.swing.JFrame {
    ...
    public Login() {
        initComponents();
        setIcon();
        new FillCombo().fillCombo("SELECT * FROM login ORDER BY username", combo_user, "username");
    }
    ...
    private void initComponents() {
        ...
        jPanel2.add(jButton4, new org.netbeans.lib.awtextra.AbsoluteConstraints(770, 0, 40, 40));
        ...
        jLabel5.setText("Security System");
        jPanel2.add(jLabel5, new org.netbeans.lib.awtextra.AbsoluteConstraints(30, 10, 310, 50));
        ...
        jPanel2.add(jButton3, new org.netbeans.lib.awtextra.AbsoluteConstraints(120, 320, 140, 30));
        ...
        jPanel2.add(jLabel4, new org.netbeans.lib.awtextra.AbsoluteConstraints(370, 0, 450, 450));
    ...
    private void txt_passKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_passKeyReleased
        if (evt.getKeyCode() == KeyEvent.VK_ENTER) {// press enter to login
            login();
        }
    }//GEN-LAST:event_txt_passKeyReleased

    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        login();// login by clicking on "login"
    ...
    private void jButton4ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton4ActionPerformed
        System.exit(0); // click red circle to exit
    ...
    public void login() {
        db.connect();
        try {
            db.runSQL("SELECT * FROM login WHERE username='" + combo_user.getSelectedItem() + "'");
            db.rs.first();
            if (db.rs.getString("password").equals(txt_pass.getText())) { //check password if its the same as in database
                new Menu().setVisible(true);// if true open the main menu
                this.setVisible(false);
                new Settings(this, true);
            } else {
                message_txt.setText("Invalid Password!"); // show message when entering a wrong password
                message_txt.setForeground(new Color(202, 66, 66));
            }
            
        } catch (NullPointerException | SQLException ex) {
            message_txt.setText("User does not exists!"); // show message when entering a wrong user or sql error
            message_txt.setForeground(new Color(202, 66, 66));
        }
        db.disconnect();
    }
    ...
    private void setIcon() {
        setIconImage(Toolkit.getDefaultToolkit().getImage(getClass().getResource("/Images/icone.png"))); // set an icon for the app
    }
